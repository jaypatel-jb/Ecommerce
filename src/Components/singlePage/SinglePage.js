        ...
        <Grid item lg={8} md={8}>
          <Box sx={User_Action}>
            {innerWidth <= 902 ? null : (
              <Box sx={Style}>
                <Tooltip title="Grid view">
                  <IconButton
                    className="GridViewRoundedIcon"
                    onClick={() => setGrid_List(true)}
                  >
                    <GridViewRoundedIcon />
                  </IconButton>
                </Tooltip>
                ...
                <Tooltip title="List view">
                  <IconButton
                    className="ReorderRoundedIcon"
                    onClick={() => setGrid_List(false)}
                  >
                    <ReorderRoundedIcon />
                  </IconButton>
                </Tooltip>
            ...
            <Box color="GrayText" sx={Style}>
              <Typography variant="body1" component="span">{`No of products ${
          ...
          <Grid sx={Grid_Style} container spacing={2}>
            {Producfilterlist
              ? Producfilterlist.map((product) => {
                  return Grid_List ? (
                    <GridView key={product.id} products={product} />
                  ) : (
                    <ListView key={product.id} products={product} />
                  );
                })
              : null}
          </Grid>
